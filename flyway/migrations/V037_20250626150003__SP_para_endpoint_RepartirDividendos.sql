...
-----------------------------------------------------------
CREATE PROCEDURE [dbo].[SP_RepartirDividendos]
    @projectId INT,
...
BEGIN
    ...
    SELECT @MontoGanancias = t.amount, @PaymentID = t.payid 
    ...
    AND i.statusid = (SELECT statusid FROM cf_status_types WHERE name = 'Activo')
    AND NOT EXISTS (
        SELECT 1 
    ...
    WHERE i.projectid = @projectId
    AND ia.statusid = (SELECT statusid FROM cf_status_types WHERE name = 'Activo');
    ...
    BEGIN TRY
        ...
        IF @reporteAprobado = 0
            RAISERROR('El reporte financiero aún no está en estado aprobado', 16, 1)
        ...
        AND pfc.statusid = (SELECT statusid FROM cf_status_types WHERE name = 'Activo')
        ...
        VALUES (
            ...
            @FechaEjecucion,
            @FechaEjecucion,
            HASHBYTES('SHA2_256', CAST(@projectId AS VARCHAR) + CAST(@MontoGanancias AS VARCHAR) + CAST(@FechaEjecucion AS VARCHAR)),
            (SELECT transactiontypeid FROM vpv_transactiontypes WHERE name = 'Dividendo'),
            (SELECT transactionsubtypeid FROM vpv_transactionsubtypes WHERE name = 'Distribución Proyecto'),
        ...
        BEGIN
            ...
            VALUES (
                ...
                @ComisionAmount,
                @numeroreferencia + '-GRP-' + CAST(@GroupID AS VARCHAR), 
                @FechaEjecucion,
                @FechaEjecucion,
        ...
        BEGIN
            ...
            VALUES (
                @numeroreferencia + '-INV-' + CAST(@AgreementID AS VARCHAR), /*TODO: Ver si hace falta cambiar esto*/
                ...
                @AgreementID, 
                (SELECT portfolioid FROM cf_investment_portfolios WHERE userid = @UserID),
                @PaymentID
            );
    ...
    BEGIN CATCH
        ...
        VALUES (
            'Error en SP_RepartirDividendos',
            GETDATE(),
        ...
        RAISERROR('Error en distribución de dividendos - Consulte al administrador', 16, 1)
    END CATCH
